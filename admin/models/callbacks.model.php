...
class CallbacksModel extends Model
	...
	function ajax()
		...
		if ($_GET['ajax']=='getCallbacks')
			{
			$clbs = $this->db->getCallbacks($this->ajax_pg,$this->ajax_pp,$this->ajax_sort,@$this->ajax_qs['user_id'],@$this->ajax_qs['m_num']);
			...
			if (count($clbs)>0) foreach ($clbs as $k=>$v)
				...
				if (!isset($uss[$v['user_id']])) $uss[$v['user_id']] = $this->db->uni_select_one('users',['user_id'=>$v['user_id']]);

				$m_data = $mss[$v['m_num']];
				$clbs[$k]['m_title'] = $m_data['m_title'];

				$u_data = $uss[$v['user_id']];
				$clbs[$k]['user_email'] = $u_data['user_email'];
