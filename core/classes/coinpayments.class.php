...
class CoinPaymentsAPI {
	...
	public function __construct() {
		global $config;
		$this->Setup($config['coinpayments']['private'], $config['coinpayments']['public']);
	}
	...
	public function Get_callback_address($req) {
	...
	public function getRatesUsd() {
		...
		if (@$arr['error']=='ok')
			{
			if (isset($arr['result']['USD']))
				{
				$main_rate = 1 / $arr['result']['USD']['rate_btc'];

				foreach($arr['result'] as $k=>$v)
					{
					$ret[$k] = $v['rate_btc'] * $main_rate;
					}
				}
			}
